...
- Contact Support Card on article + section browse (`gap-10` under the article Grid); section search no-results uses inline Flatpack Link copy (`Try another keyword or {contact}.`) instead of an EmptyState Button slot
...
- Installer/agent endpoint list in `docs/api.md` (auth, routes, fields, search, examples)
...
### Changed
- Documented the Recording Studio API slice for sections and pages (`docs/api.md` endpoint list; `docs/api-plan.md` design)
...
- JSON API is optional. Add `recording_studio_api` `v0.5.5`, install/migrate, mount, and enable `:api_access_point` on roots that hold keys. Writes still need AdminRoot `:edit`. `GET support_pages?q=` uses Recording Studio Search (not Instant). Tune `api_search_rate_limit_*` if 30/minute is wrong. Endpoint list: `docs/api.md`
...
- Hosts that overrode public or staff article show body wrappers should drop Tailwind `prose` and use `support_page_body_class` (Flatpack `flat-pack-content-editor-content`) so TipTap display typography matches ContentEditor
- Load `stylesheet_link_tag "flat_pack/content_editor"` beside `flat_pack/rich_text` so the content-surface styles apply (Propshaft does not expand the `@import` inside `flat_pack/application`)
...
### Upgrade notes
- No migrations or route changes
- To show a contact button on public section pages, set `RecordingStudioSupport.configure { |c| c.public_contact_href = "…"; c.public_contact_label = "Contact support" }`
- Optional: set `public_section_subtitle` to a string or `->(section) { … }` for per-topic blurbs
- Staff `/support/sections/...` is unchanged
...
### Changed
- Dummy pins `recording_studio_user` `v0.9.0`, Accessible `v0.9.1`, Attachable `v0.5.1`, Admin `v2.0.2`, and Flatpack `v0.1.171`
...
- Public article body uses Flatpack PageTitle plus Flatpack ContentEditor content-surface classes for long-form TipTap display (no separate Content component)
