  ...
  module Api
    ...
    module MemberActionsLookup
      ...
      def authorize_action!(action, context)
        if context.recording&.recordable_type == RecordingStudioSupport::Api::PAGE_TYPE &&
      ...
      def action_params(action)
        type = begin
          resource_recording.recordable_type
        rescue RecordingStudioApi::NotFoundError
          nil
        end
        return super unless type == RecordingStudioSupport::Api::PAGE_TYPE
        ...
        filtered_params = raw_params.except(*reserved)
