...
11. Optional JSON API: add `recording_studio_api` to the **host** Gemfile (not the Support gemspec), install and mount it, and enable `:api_access_point` on roots that hold keys. Support registers `support_sections` and `support_pages`. Writes need AdminRoot `:edit`. Endpoints, auth, search, and examples: `docs/api.md`. Public `/help` stays anonymous browse.
