...
class CursorInstallTest < Minitest::Test
  ...
  def test_gemspec_excludes_cursor_and_fetched_pack_is_untracked
    spec = Gem::Specification.load(File.join(ROOT, "recording_studio_support.gemspec"))
    ...
    refute_includes fetch, "git clone"
    refute_includes fetch, "${HOME}/.cursor/skills"
    refute_includes fetch, "${HOME}/.cursor/rules"

    tracked_skills = `git -C #{ROOT} ls-files -- .cursor/skills`
    tracked_rules = `git -C #{ROOT} ls-files -- .cursor/rules`
    assert_equal "", tracked_skills.strip
    assert_equal "", tracked_rules.strip
  end
