...
class AddSearchVectorToRecordingStudioSupportPages < ActiveRecord::Migration[8.1]
  ...
  def add_search_indexes
    add_gin_index_unless_exists(
      :search_vector,
      "index_recording_studio_support_pages_on_search_vector"
    )
    add_trgm_index_unless_exists(
      :title,
      "index_recording_studio_support_pages_on_title_trgm"
    )
    add_trgm_index_unless_exists(
      :body,
      "index_recording_studio_support_pages_on_body_trgm"
    )
  end
