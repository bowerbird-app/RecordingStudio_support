...
3. Enable `:api_access_point` on roots that hold API keys (`Workspace` and `AdminRoot` in dummy). Writes need AdminRoot `:edit`. `GET /recording_studio_api/api/v1/support_pages?q=` searches articles via `SupportPage.search`. Default search rate limit is 30 per client per minute. Endpoint list: `docs/api.md`.
