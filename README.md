...
Public show is Publishable's published route (`/help/:uuid/:slug`). Header order is Flatpack Badge (section title, stock `size: :lg`) → `PageTitle` (title + optional `description` subtitle; wrapped to cancel stock `mb-6`) → stock Timestamp (publish/updated day) → long-form body in `prose` (Flatpack’s text/content pattern — there is no Content component). Body `ol`/`ul` render through Flatpack `List`; wrap secondary tip copy in `blockquote` so it uses muted theme tokens. The document `<title>` is the page title. Meta description prefers the page `description` when set; otherwise plain text from the body (HTML stripped, entities decoded, max 160 characters). There is no Related list. PageNav backs to the section (or `/help`) and offers Home to `/help` (no Close). No live banner, no sign-in alert, no Edit, trash, or Access. Do not wrap the title stack or body in an extra card.
