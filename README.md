...
Elevated + `hover: :strong` needs a host Tailwind utilities-layer override: Flatpack’s hover rules live in `@layer components`, but elevated also applies Tailwind utilities (`shadow-md`, `border-[var(--card-border-color)]`), and utilities win on hover. Dummy’s `app/assets/tailwind/application.css` shows the override hosts should copy until Flatpack moves those hover rules.
